...
if __name__ == '__main__':
    ...
    flag = raw_input(u"是否继续？(y/n)".encode("gbk"))
    ...
    try:
        ...
        if(match):
            print(u"欢迎您,".encode("gbk") + match.group(3).decode("utf-8").encode("gbk"))
        else:
            errorExit(u"登录失败，请核对用户名密码重启软件")
        ...
        print(u"您已选" + str(len(classList)) + u"门课：")
        # for c in classList:
        #    print(c[1] + "(" + c[3] + ")")
        for c in classList:
            url = c[2]
        ...
        errorExit(u"妈呀，出错了，请重启软件重试")
                
    print("\n")
    errorExit(u"课件下好了，滚去学习吧！\n")
